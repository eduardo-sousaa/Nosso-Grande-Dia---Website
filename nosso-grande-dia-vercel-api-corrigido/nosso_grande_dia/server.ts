...
async function startServer() {
  if (process.env.NODE_ENV !== 'production') {
    ...
    // React SPA fallback for local Node production runs.
    // Vercel serves the Vite build as static files and routes /api/* to the
...
// In Vercel, the Express app is exported from api/[...path].ts and must not
// call app.listen(). Local development still starts the normal Node server.
